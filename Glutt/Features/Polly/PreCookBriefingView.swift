...
struct PreCookBriefingView: View {
    ...
    /// How much Chef interferes, offered only to cooks who are actually wearing
    /// the camera.
    ///
    /// Same chip row as the chef picker directly below it, because they are the
    /// same kind of decision: both are fixed for the session, both are about who
    /// is in your ear rather than what you are making, and both belong in the
    /// last few seconds before a cook rather than buried in settings.
    /// Names only, with the chosen level explained underneath.
    ///
