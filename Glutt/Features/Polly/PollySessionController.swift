...
final class PollySessionController {
    ...
    func start(context: ModelContext, requireMic: Bool = true) async {
        ...
        registry.seesContinuously = visuals.activeKind == .metaGlasses
    ...
    private func refreshSeeingRulesIfNeeded() async {
        ...
        // The tool-level rule follows the same flag: glasses that arrive mid-cook
        // start enforcing the look, and glasses that drop stop demanding one the
        // cook can no longer give.
        registry?.seesContinuously = seesNow
        do {
            try await transport.send(.sessionUpdate(config))
            PollyDebugLog.shared.log(
