...
final class SubscriptionGate {
    ...
    /// Clears `isPresenting` when no handler callback ever lands. See
    /// `presentPaywall()`.
    ...
    func presentPaywall() {
        ...
        //
        // A stuck `true` leaves the locked cover with nothing behind it: every
        // bounce tap is swallowed by the guard above and the wall never comes
        // back until the app is relaunched. That is not hypothetical — on the
        // simulator Superwall cannot load StoreKit products, so a presentation
        // can neither show nor report, and the first tap kills the rest.
